...
void LoadBTree()
{
	...
	bTree = new bTreeClass;										// create bTree object 
...
string Parse(string strIn)
{
	...
	string strWork;												// work string during parsing 
	...
	for (int i = 0; i < strIn.length(); i++)
	{
			...
			strIn[i] == ' ')
		{
			...
			{
				strReturn += lastc;									// add saved char to output 
				strWork = "";										// empty work string 
				lastc = ' ';										// clear hold char 
			}
			else													// usable character returned
				lastc = c;											// save good char
		}
	}

	if (lastc != ' ')											// is there a leftover char?
		strReturn += lastc;										// append it to a work string 

	return strReturn;											// return parsed text 
}